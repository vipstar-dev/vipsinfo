...
type IOpCodes = {
  [key in OpcodeTypes]: number
}

const opcodes: IOpCodes = {
  OP_0: 0x00,
  OP_PUSHDATA1: 0x4c,
  OP_PUSHDATA2: 0x4d,
  OP_PUSHDATA4: 0x4e,
...
class Opcode {
  ...
  toBuffer(): Buffer {
    return Buffer.from([this.code !== null ? this.code : 0])
  }
  ...
  isSmallInt(): boolean {
    const code = this.code !== null ? this.code : 0
    return (
      this.code === opcodes.OP_0 ||
      (code >= opcodes.OP_1 && code <= opcodes.OP_16)
    )
  }
  ...
  toSmallInt(): number | undefined {
    ...
    if (this.code === opcodes.OP_0) {
      return 0
    } else if (code >= opcodes.OP_1 && code <= opcodes.OP_16) {
      return code - opcodes.OP_1 + 1
    }
  }
